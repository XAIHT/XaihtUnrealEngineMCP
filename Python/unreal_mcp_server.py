...
def get_unreal_connection() -> Optional[UnrealConnection]:
    ...
    try:
        if _unreal_connection is None:
            ...
            # Each command opens a fresh socket inside send_command (Unreal closes the
            # connection after every command), so no liveness ping is needed here.
            # The previous b'\x00' ping sent a stray null byte that Unreal could not
            # parse as JSON; it has been removed.
            pass
...
def info():
    ...
    - `take_screenshot(filepath)` - Capture the active viewport to a .png on disk
    ...
    ## Level / World Tools
    - `open_level(path)` - Load a map by content path
    - `save_current_level()` / `save_all()` - Save the current level / all dirty packages
    - `new_level(path)` - Create a new blank level (optionally save it)
    - `get_current_level()` - Name, path and actor count of the current level

    ## Asset Tools
    - `import_asset(source_file, destination_path)` - Import FBX/texture/audio/etc.
    - `duplicate_asset(source, destination)` / `rename_asset(source, destination)`
    - `delete_asset(path)` / `save_asset(path)` / `create_folder(path)`
    ...
    - `build_project(editor_cmd, uproject, ...)` - Headless compile (no editor UI)
    - `run_automation_tests(test_filter, ...)` - Headless automation test run

...
`127.0.0.1:55557`, and exposes ~56 tools across 10 categories to your AI client.
...
| `blueprint_tools.py` | Blueprint | `create_blueprint`, `add_component_to_blueprint`, `set_static_mesh_properties`, `set_component_property`, `set_physics_properties`, `compile_blueprint`, `set_blueprint_property`, `set_pawn_properties` |
...
| `system_tools.py` | System / introspection | `execute_python`, `execute_console_command`, `get_class_info`, `list_assets`, `call_unreal` |
...
| `material_tools.py` | Materials | `create_material`, `create_material_instance`, `set_material_parameter`, `assign_material` |
...
1. **C++ (plugin) side.** Add a handler method to the matching
   ...
   pattern), and route its command name in `UnrealMCPBridge::ExecuteCommand`.

    ...
    while (bRunning)
    {
        ...
        if (ListenerSocket->HasPendingConnection(bPending) && bPending)
        {
            ...
            if (ClientSocket.IsValid())
            {
                ...
                // Accumulate bytes until we have a complete JSON message. The Python
                // client sends one JSON object per connection with no delimiter, so we
                // parse-until-valid rather than assuming a single Recv yields the whole
                // message (this fixes truncation for payloads larger than the buffer).
                uint8 Buffer[8192];
                FString MessageBuffer;
                while (bRunning)
                {
                    int32 BytesRead = 0;
                    // Read at most sizeof(Buffer)-1 bytes so we can safely null-terminate.
                    if (ClientSocket->Recv(Buffer, sizeof(Buffer) - 1, BytesRead))
                    {
                        ...
                        // Append received chunk to the accumulation buffer
                        Buffer[BytesRead] = '\0';
                        MessageBuffer.Append(UTF8_TO_TCHAR(Buffer));

                        // Try to parse what we have so far. If it is not yet a complete
                        // JSON object, keep reading more chunks.
                        TSharedPtr<FJsonObject> JsonObject;
                        TSharedRef<TJsonReader<>> Reader = TJsonReaderFactory<>::Create(MessageBuffer);
                        if (!FJsonSerializer::Deserialize(Reader, JsonObject) || !JsonObject.IsValid())
                        {
                            UE_LOG(LogTemp, Verbose, TEXT("MCPServerRunnable: Incomplete JSON (%d chars so far), waiting for more"), MessageBuffer.Len());
                            continue;
                        }

                        // Complete message received; consume the buffer.
                        MessageBuffer.Empty();
                        ...
                        {
                            // Parameters are optional; default to an empty object instead
                            // of dereferencing a missing field.
